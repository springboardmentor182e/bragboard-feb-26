  ...
  const modalContent = (
    <>
      ...
      >
        <div
          ref={modalRef}
          className="bg-white dark:bg-slate-900 w-full max-w-[650px] max-h-[90vh] overflow-y-auto rounded-2xl shadow-2xl transition-transform duration-300"
          style={{ transform: isAnimating ? "scale(1)" : "scale(0.95)" }}
          onClick={(e) => e.stopPropagation()}
        >
          {/* HEADER */}
          <div className="sticky top-0 flex justify-between items-center px-6 py-4 border-b dark:border-slate-800 bg-white dark:bg-slate-900 rounded-t-2xl z-10">
            <div>
              <h2 id="create-shoutout-title" className="text-xl font-semibold text-slate-900 dark:text-white">Create a Shout-Out</h2>
            ...
            <button
              onClick={onClose}
              disabled={loading}
              className="flex-shrink-0 hover:bg-slate-100 dark:hover:bg-slate-800 p-1 rounded transition disabled:opacity-50 text-slate-500 dark:text-slate-400"
          ...
          <div className="p-6 space-y-6 text-slate-900 dark:text-slate-100">
                ...
                {badges.map((b, i) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium text-sm text-slate-900 dark:text-white">{b.title}</p>
            ...
            <div>
              ...
              <div className="relative mt-2">
                <input
                  ...
                  className="w-full px-4 py-2 border dark:border-slate-800 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 dark:bg-slate-800 dark:text-white disabled:bg-slate-100 dark:disabled:bg-slate-950"
                />

                {/* DROPDOWN */}
                {showUserDropdown && searchQuery && filteredUsers.length > 0 && (
                  <div className="absolute top-full mt-1 w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-lg z-50 max-h-48 overflow-y-auto">
                    {filteredUsers.map((user) => (
                      <button
                        key={user.id}
                        onClick={() => handleAddUser(user)}
                        type="button"
                        className="w-full text-left px-4 py-3 hover:bg-slate-100 dark:hover:bg-slate-700 border-b dark:border-slate-700 last:border-b-0 transition"
                      >
                ...
                {showUserDropdown &&
                  ...
                  allUsers.length > 0 && (
                    <div className="absolute top-full mt-1 w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-lg z-50 p-4">
              ...
              {selectedUsers.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-3">
                  {selectedUsers.map((user) => (
                    <div
                      key={user.id}
                      className="flex items-center gap-2 px-3 py-1 bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300 rounded-full text-sm"
                    >
                      <span>{user.name}</span>
                      <button
            ...
            {/* POINTS SELECTION */}
            <div>
              <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Points to Award</label>
                ...
                {POINTS_OPTIONS.map((option) => (
                  <button
                    ...
                    className={`
                      ...
                      ${
                        points === option.value
                          ? "border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-400 dark:border-indigo-800"
                          : "border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300"
                      }
                      ${loading ? "opacity-50 cursor-not-allowed" : ""}
                    `}
                  >
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full mt-2 px-4 py-3 border dark:border-slate-800 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 dark:bg-slate-800 dark:text-white disabled:bg-slate-100 dark:disabled:bg-slate-950"
              />
            ...
            <div className="flex justify-end gap-3 pt-4 border-t dark:border-slate-800">
              <button
                onClick={onClose}
                disabled={loading}
                className="px-4 py-2 rounded-xl border dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 disabled:opacity-50 transition"
              >
                Cancel
              </button>
