...
const FeedCard = ({ item, onShoutoutDelete }) => {
  ...
  return (
    ...
    >
      ...
      {/* Subtle glow */}
      <div className="absolute -top-6 -right-6 w-24 h-24 bg-indigo-400/10 dark:bg-indigo-400/5 blur-2xl opacity-0 group-hover:opacity-100 transition" />
      ...
      <div className="flex flex-wrap items-center gap-2 mt-4">
        {item.category && (
          <div className="inline-block text-xs px-3 py-1 rounded-lg border bg-indigo-100 dark:bg-indigo-900/40 text-indigo-600 dark:text-indigo-400 border-indigo-200 dark:border-indigo-800 font-medium">
            {item.category}
          </div>
        )}
      ...
      <p className="text-sm text-slate-700 dark:text-slate-300 mt-4 leading-relaxed">
        ...
        {item.message?.length > 120 && (
          <span
            ...
            className="ml-2 text-indigo-600 dark:text-indigo-400 cursor-pointer text-xs font-medium hover:underline"
          >
      ...
      {canEdit && (
        <div className="flex gap-2 mt-3 pb-2 border-b border-slate-100 dark:border-slate-800">
          <button
            onClick={(e) => {
              e.stopPropagation();
            ...
            className="flex items-center gap-1 px-3 py-1 text-xs bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/40 rounded-lg transition"
            title="Edit shoutout"
          >
            <Edit2 size={14} /> Edit
          </button>
          <button
            ...
            disabled={isDeleting}
            className="flex items-center gap-1 px-3 py-1 text-xs bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/40 disabled:opacity-50 rounded-lg transition"
            title="Delete shoutout"
          >
            <Trash2 size={14} /> Delete
          </button>
          <div className="ml-auto text-xs text-amber-600 dark:text-amber-400 font-semibold flex items-center">
      ...
      <div className="mt-5 space-y-3">
        ...
        <div className="flex items-center gap-3 relative">
          ...
          <div className="relative">
            ...
            {/* REACTION PICKER */}
            {showReactionPicker && (
              <div className="absolute bottom-12 left-0 bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 shadow-lg p-2 flex gap-2 z-50">
                <button
          ...
          <button
            ...
            className="
              flex items-center gap-2 px-3 py-1.5 rounded-full text-sm 
              transition-all duration-200
              bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:bg-red-50 dark:hover:bg-red-900/20 hover:text-red-600 dark:hover:text-red-400
              hover:border hover:border-red-200 dark:hover:border-red-800
            "
      ...
      {isEditModalOpen && createPortal(
        <>
          ...
          />
          <div 
            className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white dark:bg-slate-900 rounded-2xl shadow-2xl z-50 p-6 space-y-4 border border-slate-200 dark:border-slate-800 transition-colors"
            onClick={(e) => e.stopPropagation()}
          >
            <h3 className="text-lg font-bold text-slate-900 dark:text-white">Edit Shout-Out</h3>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-3 py-2 border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 text-sm transition-colors"
                rows="4"
              />
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 py-2 rounded-lg font-medium hover:bg-slate-300 dark:hover:bg-slate-700 transition"
              >
                Cancel
              </button>
            </div>
