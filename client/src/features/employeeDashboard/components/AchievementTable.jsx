  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-slate-200 dark:border-gray-700 p-6">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          onClick={() => { setEditingAchievement(null); setIsOpen(true); }}
              ...
              {achievements.map((item) => (
                ...
                >
                  <td className="py-4 font-semibold text-slate-800 dark:text-slate-100">
                    {item.title}
                    {item.description && (
                      <p className="text-xs font-normal text-slate-400 mt-0.5">{item.description}</p>
                    )}
                  </td>
                  ...
                  <td>
                    <span className={`text-xs font-medium px-2 py-1 rounded-full ${statusClass[item.status] ?? "bg-gray-100 text-gray-600"}`}>
                      {item.status ?? "Pending"}
                    </span>
                  </td>
                  <td className="text-right">
                    <div className="flex justify-end gap-4">
                      <button
                        onClick={() => { setEditingAchievement(item); setIsOpen(true); }}
